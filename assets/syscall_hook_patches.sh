#!/bin/bash
# Patches author: backslashxx @ Github (minimal syscall hooks)
...
# Vendored into C9 builder for KernelSU-Next legacy (non-GKI) manual hook integration.
# This is the RECOMMENDED hook method by KSU-Next dev (rifsxd) for non-GKI + latest Manager.
# Do NOT combine with official/tiann/kucingoranye manual hook patches.
    ...
    security/selinux/ss/services.c)
        if grep -q "selinux_state" "security/selinux/include/security.h" >/dev/null 2>&1; then
            ...
            if ! grep -q "static DEFINE_RWLOCK" "security/selinux/ss/services.c"; then
                ...
                echo "[-] security/selinux/ss/services.c patch failed for unknown reasons, please provide feedback in time."
            fi
        fi
